...
- **实时令牌统计**：输入框上方状态条实时显示本次生成的 输出 token / 耗时 / TPS / 输入 token，控制台内显示会话进程累计用量（启发式估测）

### 🔀 Git 变更（SCM）
- 真实 `git status` 变更面板：已暂存 / 未暂存 / 未跟踪 三组，显示分支名
- 单文件「暂存 / 取消暂存 / 丢弃」+「全部暂存」，操作后自动刷新

### 📂 文件浏览与预览
- 浏览工作区目录树（文件夹优先排序），点击进入子目录
- 点击文件即时预览：图片（≤4MB，直接渲染）与文本/代码（≤256KB）

### 🎨 皮肤中心
- 6 款皮肤：亮色 / 暗夜 / 海洋 / 森林 / 日落 / 水墨，即点即换、本地持久化
- 控制台面板支持拖拽移动 + **左缘拖拽调宽**（宽度记忆）
...
### 🔧 页面增强
- 输入框上方**实时状态条**：智能体 / 运行 / 任务 / 图库 计数 + 令牌统计
- 侧边栏底部 **🧭 控制台** 入口（运行数红色徽章）
- 可拖拽、可调宽的控制台面板（随皮肤主题变色）
...
- 「Git 变更」页签依赖 shell 服务执行 `git`：部分会话沙箱禁止 spawn 外部程序，此时自动降级为「分支名来自 `.git/HEAD` + 变更列表提示不可用」，其余功能不受影响
